...
Additional options:
  ...
  -l, --list      Path to the lista_seq.tsv file.
  -d, --dir       Base working directory to save the analysis.
  -h, --help      Show this help message and exit.

Example:
  ./aluminion.sh -r BAC_2025_NOV_25 -b /mnt/data/Databases -t 60 
...
if [ ! -f "lista_seq.tsv" ]; then
    ...
    error_log "Empty 'lista_seq.tsv' created. Fill it out and re-run."
    exit 1
fi
...
# PIPELINE EXECUTION
# ==============================================================================
# 1. Base Environment (QC, Filtering, Assembly, Kraken, Bakta, Abricate, etc.)
conda activate aluminion_base

log "Pre-filtering QC..."
for i in $(cat samples); do NanoPlot --fastq 01_reads/${i}.fastq.gz -o 01_reads/QC/${i} --downsample 20000 --threads 3 & done; wait
...
for i in $(cat samples); do NanoPlot --fastq 02_filter/${i}.fastq.gz -o 02_filter/QC/${i} --downsample 20000 --threads 3 & done; wait
...
for i in $(cat samples); do docker run --rm -v $(pwd):/mnt/ --user $(id -u):$(id -g) "kbessonov/mob_suite:3.0.3" mob_recon -i /mnt/03_assemblies/${i}.fasta -o /mnt/08_Anotacion/${i}/mob_recon -c --force -n $THREADS_TOTAL; done
